...
public class GameManagerSP : MonoBehaviour
{
    ...
    private bool intitialShuffle = true;
    ...
    void Update()
    {
        ...
        if (!shuffling && CheckCompletion() && intitialShuffle)
        {
            shuffling = true;
            StartCoroutine(WaitShuffle(0.5f));
            intitialShuffle = false;
        }

        else if(!shuffling && CheckCompletion() && !intitialShuffle)
        {
            GameManager.instance.SetMinigameComplete("Sliding Puzzle");
